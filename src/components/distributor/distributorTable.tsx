...
const DistributorTable = () => {
  ...
  const [editDistributor, setEditDistributor] = useState<Distributor | null>(
    null
  );
  ...
  const confirmDeleteDistributor = async () => {
    ...
    try {
      ...
      setDistributors((prev) =>
        prev.filter((d) => d._id !== selectedDistributorId)
      );
              ...
              <div className="fixed inset-0 flex items-center justify-center p-2">
                <Dialog.Panel className="w-full max-w-4xl rounded-lg bg-white shadow-lg">
                  <div className="flex justify-between py-[19px] px-[25px]">
                    <p className="text-2xl font-bold text-[#232D42]">
                      Add Distributor
                    </p>
                    <button
          ...
          <table className="w-full bg-white overflow-x-auto">
            <thead className="bg-white">
              <tr
                ...
                style={{ borderTopColor: "#e9ecef" }}
              >
                <th
                  scope="col"
                  className="py-3.5 px-4 font-semibold text-left rtl:text-right text-black "
                >
                  Distributors
                </th>
                <th
                  scope="col"
                  className="px-4 py-3.5 font-semibold text-left rtl:text-right text-black"
                >
                  Country/State
                </th>
                <th
              ...
              {loading ? (
                ...
                paginatedDistributors.map((item, index) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-4 text-sm text-gray-500">
                      <span
                        className={`p-1 px-2 rounded-2xl font-medium ${
                          item.status === "active"
                        ...
                        <Popover className="relative cursor-pointer">
                          {({ close }) => (
                            <>
                              ...
                              <Popover.Panel
                                className="absolute right-0 z-10 mt-2 w-50 bg-white shadow-lg border rounded-md"
              ...
              <div className="fixed inset-0 flex w-screen items-center justify-center p-4 bg-black/40 dialog-item dialog-notification dialog-email-success">
                <DialogPanel className="max-w-full w-xl space-y-4 border bg-white p-6 rounded-xl dialog-panel">
                  <div className="dialog-body">
                    ...
                    <p>
                      Are you sure you want to delete this distributor? This
