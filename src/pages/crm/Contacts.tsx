...
import { ApprovedEmailIcon, DialogDeleteIcon, DialogueCheckMarkIcon } from '../../components/svg/icons';
...
export default function Contacts() {
  ...
  const totalPages = Math.ceil(crmUsers.length / itemsPerPage);
    ...
    const [isDeleteItemConfirmation,setIsDeleteItemConfirmation] = useState(false);
    ...
    return (
                  ...
                  <Popover className="action-drop">
                    ...
                    >
                      <div className="action-menu">
                        <a href="javascript:void(0)" onClick={()=>{setIsEditContactItem(true)}} className="action-menu-item">
                          <p>Edit</p>
                        </a>
      ...
      <div className='table-row'>
        <div className='table-cell'>
          <Button className='btn-primary' onClick={()=>{setIsEmailSentSuccess(true)}}>Email Success</Button>
          <Button className='btn-primary-outline' onClick={()=>{setIsDeleteItemConfirmation(true)}}>Delete</Button>
        </div>
        ...
        <Dialog open={isEditContactItem} onClose={() => setIsEditContactItem(false)} className="relative z-50">
            <div className="fixed inset-0 flex w-screen items-center justify-center p-4 bg-black/40 dialog-item dialog-item-edit-contact">
                <DialogPanel className="max-w-full w-4xl space-y-4 border bg-white p-6 rounded-xl dialog-panel">
                    <div className="dialog-header">
                        <h3>Edit Contact</h3>
                        <Button className='closeBtn' onClick={() => setIsEditContactItem(false)}>
                    ...
                    <div className="dialog-body">
                        <div className="grid grid-cols-2 gap-4">
                          ...
                          <div>
                            <Field className="fieldDv">
                                <Label>Date</Label>
                                <Calendar onChange={item => setDate(item)} date={date} color={'#B92825'} />
                            </Field>
                          </div>
                        </div>
                    </div>
                    <div className="dialog-footer">
                        ...
                        <Button className="btn btn-primary" onClick={() => setIsEditContactItem(false)}>Save Changes</Button>
                    </div>
                </DialogPanel>
            </div>
        </Dialog>

        <Dialog open={isEmailSentSuccess} onClose={() => setIsEmailSentSuccess(false)} className="relative z-50">
            <div className="fixed inset-0 flex w-screen items-center justify-center p-4 bg-black/40 dialog-item dialog-notification dialog-email-success">
                <DialogPanel className="max-w-full w-xl space-y-4 border bg-white p-6 rounded-xl dialog-panel">
                    ...
                    <div className="dialog-body">
                      <Button className='closeBtn' onClick={() => setIsEmailSentSuccess(false)}>
                        <FontAwesomeIcon icon={faXmark} />
                      </Button>
                      ...
                      <p>Your email has been successfully delivered to the recipient. Please check your "Sent" folder for confirmation or await a response.</p>
                      
                      <div className="btn-row">
                        <Button className="btn btn-primary" onClick={() => setIsEmailSentSuccess(false)}>Okay, Great!</Button>
                      </div>
        ...
        <Dialog open={isDeleteItemConfirmation} onClose={() => setIsDeleteItemConfirmation(false)} className="relative z-50">
            <div className="fixed inset-0 flex w-screen items-center justify-center p-4 bg-black/40 dialog-item dialog-notification dialog-email-success">
                <DialogPanel className="max-w-full w-xl space-y-4 border bg-white p-6 rounded-xl dialog-panel">
                    
                    <div className="dialog-body">
                      <Button className='closeBtn' onClick={() => setIsDeleteItemConfirmation(false)}>
                        <FontAwesomeIcon icon={faXmark} />
                      </Button>
                      ...
                      <p>Before you proceed, double-check your decision to delete. This action cannot be undone, and you may lose important data permanently.</p>
                      
                      <div className="btn-row">
                      <Button className="btn btn-primary-outline" onClick={() => setIsDeleteItemConfirmation(false)}>Not Now</Button>
                        <Button className="btn btn-primary" onClick={() => setIsDeleteItemConfirmation(false)}>Yes, Delete</Button>
                      </div>
